...
const createCards = function(team) {
    ...
    ${team.filter(employee => employee.getRole() === 'Engineer').map(({name, id, email, github}) => {
        return `
                <div class="card" style="margin: 3%; max-width: fit-content;">
                    <div class="card-header" style="background-color: #0077f7;">
                        <h2 style="color:white">${name}</h2>
    ...
    ${team.filter(employee => employee.getRole() === 'Intern').map(({name, id, email, school}) => {
        return `
                <div class="card" style="margin: 3%; max-width: fit-content;">
                    <div class="card-header" style="background-color: #0077f7;">
                        <h2 style="color:white">${name}</h2>
                    ...
                    <div class="card-body">
                        <ul class="list-group">
                            <li class="list-group-item">ID: ${id}</li>
                            <li class="list-group-item">Email: <a href="mailto:${email}">${email}</a> </li>
                            <li class="list-group-item">Attending: ${school}</li>
                        </ul>
                    </div>
                </div>
            `
